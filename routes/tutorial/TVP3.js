...
router.post("/select", function (req, res) {
  ...
  const query = "select * from user_T3 where user_id = ?";
...
router.post("/insert", (req, res) => {
  ...
  const selectQuery = "SELECT * FROM user_T3 WHERE user_id = ?";
  db.query(
    selectQuery,
    [user_id],
    (selectError, selectResults, selectFields) => {
      ...
      if (selectResults.length > 0) {
        ...
        const deleteQuery = "DELETE FROM user_T3 WHERE user_id = ?";
        db.query(
          deleteQuery,
          [user_id],
          (deleteError, deleteResults, deleteFields) => {
            ...
            user_checkData.forEach((data) => {
              const query =
                "INSERT INTO user_T3 (user_id, user_checkData) VALUES (?, ?)";
